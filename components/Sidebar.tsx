...
export default function Sidebar({
  ...
  return (
    <aside
      className={`bg-black text-white h-screen flex flex-col flex-shrink-0 transition-all duration-200 ease-in-out ${
        isOpen ? "w-64" : "w-12"
      }`}
    >
      {/* Header */}
      ...
      >
        <button
          type="button"
          onClick={onToggle}
          className="w-8 h-8 bg-white/10 rounded-lg flex items-center justify-center text-white transition-colors hover:bg-white/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/40"
        ...
        <>
          <div className="px-4">
            <div className="relative">
              <MagnifyingGlassIcon className="w-4 h-4 text-white/50 absolute left-3 top-1/2 -translate-y-1/2" />
              <input
                ref={searchInputRef}
                value={searchTerm}
                onChange={(event) => setSearchTerm(event.target.value)}
                placeholder="Search"
                className="w-full bg-black/60 border border-white/20 rounded-md py-2 pl-9 pr-3 text-xs text-white placeholder:text-white/40 focus:outline-none focus-visible:ring-2 focus-visible:ring-white/30"
              />
            </div>
          </div>
